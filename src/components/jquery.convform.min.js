...
(SingleConvState.prototype.hasNext = function () {
        ...
        if (
          ...
          question =
            replacement.length >= index
              ? question.replace(ansWithin[key], replacement[index])
              : question.replace(
                  ansWithin[key],
  ...
  (ConvState.prototype.printAnswers = function (answers, multiple) {
    var opened = !1;
    if (
      (0 != this.wrapper.find("div.options").height() && (opened = !0),
  ...
  (ConvState.prototype.answerWith = function (answerText, answerObject) {
    this.current.input.hasOwnProperty("name") &&
      ...
      "password" == this.current.input.type &&
        (answerText = answerText.replace(/./g, "*"));
    var message = $('<div class="message from">' + answerText + "</div>");
    $(this.wrapper).find("div.options div.option").remove();
    var diff = $(this.wrapper).find("div.options").height(),
      originalHeight = $(this.wrapper)
        .find(".wrapper-messages")
        .data("originalHeight");
    $(this.wrapper)
      .find(".wrapper-messages")
      ...
      $(this.wrapper).find(this.parameters.inputIdHashTagName).focus(),
      answerObject.hasOwnProperty("callback") &&
      ...
      var parameters = $.extend(
          ...
          {},
          {
            ...
            eventList: {
              ...
              onInputSubmit: function (convState, readyCallback) {
                convState.current.input.hasOwnProperty("callback")
                  ? "string" == typeof convState.current.input.callback
        ...
        inputs = $(this)
          ...
          .map(function () {
            var input = {};
            return (
              $(this).attr("name") && (input.name = $(this).attr("name")),
              $(this).attr("data-no-answer") && (input.noAnswer = !0),
              $(this).attr("required") && (input.required = !0),
              $(this).attr("type") && (input.type = $(this).attr("type")),
              (input.questions = $(this).attr("data-conv-question").split("|")),
              $(this).attr("data-pattern") &&
              ...
              $(this).is("select") &&
                ...
                (input.answers = $(this)
                  ...
                  .get()),
                $(this).prop("multiple")
                  ? ((input.multiple = !0), (input.selected = []))
                  : ((input.multiple = !1), (input.selected = ""))),
              $(this).parent("div[data-conv-case]").length &&
                ((input.case = $(this)
          ...
          $(inputForm)
            ...
            .keypress(function (e) {
              if (13 == e.which) {
                ...
                if (
                  (e.preventDefault(),
                  "select" != state.current.input.type ||
                  ...
                  if (
                    ...
                    var results;
                    if ("" != input.trim())
                      (results = state.current.input.answers.filter(function (
                        el
                      ) {
                        return (
                          -1 !=
                          el.text.toLowerCase().indexOf(input.toLowerCase())
                        );
                      })).length
                        ? -1 ==
                          state.current.input.selected.indexOf(results[0].value)
                          ? (state.current.input.selected.push(
                              results[0].value
                            ),
                            state.wrapper
                              .find(parameters.inputIdHashTagName)
                      ...
                      state.current.input.selected.length &&
              ...
              autosize.update(
                $(state.wrapper).find(parameters.inputIdHashTagName)
              );
            })
            .on("input", function (e) {
              if ("select" == state.current.input.type) {
                ...
                var reg;
                new RegExp(state.current.input.pattern, "i").test($(this).val())
                  ? state.wrapper
                      .find(parameters.inputIdHashTagName)
                      .removeClass("error")
                  : state.wrapper
              ...
              if (
                ...
                if (
                  ...
                  if (
                    ...
                    0 === state.current.input.selected.length
                  )
                    return !1;
                  var results;
                  "" != input.trim() && input != parameters.placeHolder
                    ? (results = state.current.input.answers.filter(function (
                        el
                      ) {
                        return (
                ...
                if (
                  ...
                  return !1;
                var results;
                input == parameters.placeHolder && (input = ""),
                  (results = state.current.input.answers.filter(function (el) {
                    return (
                      -1 != el.text.toLowerCase().indexOf(input.toLowerCase())
                    );
